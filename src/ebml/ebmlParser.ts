...
export default class EbmlParser {
    ...
    public getParser(): EbmlParser {
        return !this._file || this._dataSize === undefined
            ? undefined
            : new EbmlParser(this._file, this._dataOffset, this._dataSize);
    }
     ...
     */
    public read(): boolean {
        if (this._offset >= (this._file.length - 1)) {
